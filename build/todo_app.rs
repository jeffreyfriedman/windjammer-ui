...
pub fn start() {
    ...
    let ui = Container::new().max_width("600px").child(Flex::new().direction(FlexDirection::Column).gap("16px").child(Text::new("📝 Todo App".to_string()).size(TextSize::XLarge).bold()).child(Flex::new().direction(FlexDirection::Row).gap("8px").child(Input::new().placeholder("What needs to be done?").value(input_display.get())).child(Button::new("Add".to_string()).variant(ButtonVariant::Primary).on_click(move || {
    ...
    }))).child(Text::new(format!("{} items", todos_display.get().len())).size(TextSize::Small)));
