...
pub fn start() {
    ...
    let ui = Container::new().max_width("600px").child((Flex::new().direction(FlexDirection::Column).gap("20px").child((Text::new("📬 Contact Us".to_string()).size(TextSize::XLarge).bold()).to_vnode()).child((Text::new("We'd love to hear from you".to_string()).size(TextSize::Medium)).to_vnode()).child((Flex::new().direction(FlexDirection::Column).gap("16px").child((Flex::new().direction(FlexDirection::Column).gap("4px").child((Text::new("Name *".to_string()).size(TextSize::Small)).to_vnode()).child((Input::new().placeholder("Your name").value(name_display.get())).to_vnode())).to_vnode()).child((Flex::new().direction(FlexDirection::Column).gap("4px").child((Text::new("Email *".to_string()).size(TextSize::Small)).to_vnode()).child((Input::new().placeholder("your.email@example.com").value(email_display.get())).to_vnode())).to_vnode()).child((Flex::new().direction(FlexDirection::Column).gap("4px").child((Text::new("Message *".to_string()).size(TextSize::Small)).to_vnode()).child((Input::new().placeholder("Your message...").value(message_display.get())).to_vnode())).to_vnode()).child((Button::new("Send Message".to_string()).variant(ButtonVariant::Primary).on_click(move || {
