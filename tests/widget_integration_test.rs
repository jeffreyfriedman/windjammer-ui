...
fn test_pbr_material_editor_widgets() {
    ...
    // 3. Slider
    let metallic_slider = Slider::new()
        .min(0.0)
        .max(1.0)
        .value(0.0)
        .render();
...
fn test_complete_material_editor_structure() {
    ...
    let _header = Flex::new()
        ...
        .child(Text::new("🎨 PBR Material Editor").size(TextSize::Large).render())
        .child(Spacer::flexible().render())
        .child(Button::new("Preview").variant(ButtonVariant::Ghost).render());
    
    // Properties panel
    let _properties = ScrollArea::vertical()
        .max_height("600px")
        .children(vec![
            // Base Color
            CollapsibleSection::new("Base Color")
                .expanded(true)
                .children(vec![
                    ColorPicker::new([1.0, 1.0, 1.0, 1.0]).render(),
                ])
                .render(),
            ...
            CollapsibleSection::new("Metallic")
                .expanded(true)
                .children(vec![
                    Slider::new().min(0.0).max(1.0).value(0.0).render(),
                ])
                .render(),
            ...
            CollapsibleSection::new("Roughness")
                .expanded(true)
                .children(vec![
                    Slider::new().min(0.0).max(1.0).value(0.5).render(),
                ])
                .render(),
    ...
    // Actions
    let _actions = Flex::new()
        .direction(FlexDirection::Row)
        .children(vec![
            Button::new("Save Material").variant(ButtonVariant::Primary).render(),
            Button::new("Load Material").variant(ButtonVariant::Secondary).render(),
