...
fn test_undo_redo_file_edit_command() {
    ...
    assert!(manager.execute(cmd).is_ok());
    assert_eq!(
        manager.get_undo_description(),
        Some("Edit test.wj".to_string())
    );

    assert!(manager.undo().is_ok());
    assert_eq!(
        manager.get_redo_description(),
        Some("Edit test.wj".to_string())
    );
}
...
fn test_undo_redo_create_delete_object() {
    ...
    // Create object
    let create_cmd =
        CommandBuilder::create_object("Enemy", "{type: \"Enemy\", health: 100}".to_string());
    assert!(manager.execute(create_cmd).is_ok());

    // Delete object
    let delete_cmd =
        CommandBuilder::delete_object("Enemy", "{type: \"Enemy\", health: 100}".to_string());
...
fn test_undo_redo_property_change() {
    ...
    let cmd =
        CommandBuilder::property_change("Player", "health", "100".to_string(), "150".to_string());

    assert!(manager.execute(cmd).is_ok());
    assert_eq!(
        manager.get_undo_description(),
        Some("Change Player.health".to_string())
    );
}
...
fn test_undo_redo_multiple_operations() {
    ...
    let cmd3 =
        CommandBuilder::property_change("Enemy", "speed", "5.0".to_string(), "10.0".to_string());
...
mod gizmo_tests {
    ...
    fn test_transform_scale() {
        let mut transform = Transform::new().with_scale(1.0, 1.0, 1.0);
...
fn test_undo_redo_with_transform_workflow() {
    ...
    assert!(
        undo_count >= 1 && undo_count <= 3,
        "Expected 1-3 operations, got {}",
        undo_count
    );
    ...
    assert!(
        redo_count >= 1 && redo_count <= 3,
        "Expected 1-3 redo operations, got {}",
        redo_count
    );
...
fn test_complete_editor_workflow() {
    ...
    let create = CommandBuilder::create_object("Enemy1", "{type: \"Enemy\"}".to_string());
    ...
    // 3. Edit a property
    let prop =
        CommandBuilder::property_change("Enemy1", "health", "100".to_string(), "150".to_string());
    ...
    let new_obj = CommandBuilder::create_object("Enemy2", "{type: \"Enemy\"}".to_string());
