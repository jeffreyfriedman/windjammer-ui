...
pub fn new(code: String) -> CodeBlock {
        CodeBlock { code: code.to_string(), language: String::from(""), show_line_numbers: false, show_copy_button: true }
}
#[inline]
...
fn render(&self) -> String {
        let language_label = {
            if !self.language.is_empty() {
                format!("<div class='wj-codeblock-language'>{}</div>", self.language.clone())
            } else {
                String::from("")
            }
        };
