...
pub fn demo() -> BtInspectorModel {
        BtInspectorModel { title: "attack".to_string(), subtitle: "action".to_string(), hotspot: "Break when reached".to_string(), params_json: "{ \"dmg\":12 }".to_string(), breakpoint: true }
}
}
...
fn render(self) -> String {
        ...
        panel = format!("{}{}", panel, "</div>");
        panel = format!("{}{}", panel, "<label class=\"bt-toggle\"><input type=\"checkbox\" ");
        panel = format!("{}{}", panel, bp);
        panel = format!("{}{}", panel, " /> Breakpoint · ");
        panel = format!("{}{}", panel, self.hotspot);
        panel = format!("{}{}", panel, "</label>");
        panel = format!("{}{}", panel, "<label class=\"bt-field\"><span>Payload</span>");
