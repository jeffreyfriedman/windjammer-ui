...
pub fn new(title: String) -> Section {
        Section { icon: "".to_string(), title: title.to_string(), children: Vec::new(), collapsed: false, accent_color: "".to_string(), removable: false, on_remove: "".to_string() }
}
#[inline]
...
fn render(&self) -> String {
        ...
        let accent_style = {
            if self.accent_color != "" {
                format!(" style='border-left: 3px solid {}'", self.accent_color.clone())
            } else {
                "".to_string()
            }
        };
        let remove_btn = {
            if self.removable {
                format!("<button class='section-remove' onclick='{}'>×</button>", self.on_remove.clone())
            } else {
                "".to_string()
            }
        };
