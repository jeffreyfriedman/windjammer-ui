...
pub fn new(label: String) -> Button {
        Button { label, variant: ButtonVariant::Primary, size: ButtonSize::Medium, disabled: false, click_handler: String::new() }
}
#[inline]
...
pub fn get_style(&self) -> String {
        ...
        let variant_style: String = match self.variant {
            ...
            ButtonVariant::Secondary => String::from(" background: #333; color: #e0e0e0; border: 1px solid #555;"),
...
fn to_vnode(&self) -> VNode {
        let mut node = VNode::button().add_class("wj-button").add_class(&self.get_variant_class()).add_class(&self.get_size_class()).add_style(&self.get_style()).set_disabled(self.disabled).add_text(&self.label);
