...
pub fn new(label: String) -> ContextMenuItem {
        ContextMenuItem { label, icon: String::new(), action: String::new(), disabled: false }
}
#[inline]
...
fn render(self) -> String {
        ...
        for item in self.items {
            ...
            { let _temp0 = format!("<button class='wj-context-item{}' onclick='{}'{}>\n                    {}\n                    <span>{}</span>\n                </button>", disabled_class, item.action, disabled_attr, icon_html, item.label); items_html.push(_temp0) };
        }
