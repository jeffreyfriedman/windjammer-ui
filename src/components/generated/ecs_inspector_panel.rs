...
use super::ecs_inspector_model::{EcsComponentSection, EcsEntityRow, EcsInspectorField, EcsInspectorSnapshot};
...
pub fn from_mock() -> EcsInspectorPanel {
        EcsInspectorPanel { snapshot: EcsInspectorSnapshot::mock_runtime_demo(), refresh_handler: "ecs_inspector_refresh()".to_string().to_string(), select_entity_handler: "ecs_inspector_select_entity".to_string().to_string() }
}
#[inline]
pub fn empty() -> EcsInspectorPanel {
        EcsInspectorPanel { snapshot: EcsInspectorSnapshot::empty(), refresh_handler: "ecs_inspector_refresh()".to_string().to_string(), select_entity_handler: "ecs_inspector_select_entity".to_string().to_string() }
}
#[inline]
...
fn render(self) -> String {
        ...
        let entities = self.snapshot.entities;
        let selected_id = self.snapshot.selected_entity_id;
        let components = self.snapshot.components;
        let selected_label = selected_entity_label_from_id(selected_id.clone());
...
#[inline]
pub fn render_entity_row(row: &EcsEntityRow, selected_id: Option<i64>, select_handler: &str) -> String {
    let row_class = entity_row_class(row.entity_id.clone(), selected_id);
    ...
    format!("<button type='button' class='{}' onclick=\"{}({})\">\n  <span class='ecs-entity-id'>#{}</span>\n  <span class='ecs-entity-badges'>{}</span>\n</button>", row_class, select_handler, row.entity_id.clone(), row.entity_id.clone(), badges)
}

#[inline]
pub fn render_entity_list(entities: Vec<EcsEntityRow>, selected_id: Option<i64>, select_handler: &str) -> String {
