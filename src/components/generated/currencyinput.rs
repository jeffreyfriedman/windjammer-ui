...
//! Regenerated from `components_wj/currencyinput.wj` — Windjammer is source of truth.
...
pub fn new() -> CurrencyInput {
        CurrencyInput { name: "amount".to_string(), input_id: "".to_string(), label: "".to_string(), currency: "USD".to_string(), value_cents: 0_i64, required: false, extra_attrs: "".to_string() }
}
#[inline]
...
fn render(&self) -> String {
        ...
        let label_html: String = {
            if self.label.is_empty() {
                ...
                format!("<label class='wj-currency-input-label' for='{}'>{}</label>", id, self.label.clone())
            }
        };
