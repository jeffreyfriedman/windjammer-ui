  ...
  window.wjAuthFetch = async function (btn) {
    ...
    try {
      ...
      if (typeof window.wjHandleScopeDeniedHint === 'function') {
        ...
        var showScope = res.status === 403 && String((data && (data.error || data.message)) || '').toLowerCase().indexOf('grant scope') >= 0;
        document.querySelectorAll('[data-wj-scope-denied]').forEach(function (el) {
          el.hidden = !showScope;
        });
      }
      if (!res.ok) {
        var errText = (typeof window.wjHttpErrorMessage === 'function')
          ? window.wjHttpErrorMessage(res.status, data, String(res.status))
          : (data.error || res.status);
        if (!auto && mount) mount.innerHTML = '<p class="err">' + errText + '</p>';
        window.lkSetSyncStatus('offline');
        return;
      }
