...
pub fn new(label: String) -> SidebarItem {
        SidebarItem { label, icon: String::new(), href: String::from("#") }
}
#[inline]
...
fn render(self) -> String {
        ...
        for item in self.items {
            ...
            { let _temp0 = format!("<a href='{}' class='wj-sidebar-item'>{}<span class='wj-sidebar-label'>{}</span></a>", item.href, icon_html, item.label); items_html.push(_temp0) };
        }
