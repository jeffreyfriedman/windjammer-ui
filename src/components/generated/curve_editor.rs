...
pub fn new(time: f32, value: f32) -> CurvePoint {
        CurvePoint { time, value, in_tangent: 0.0_f32, out_tangent: 0.0_f32 }
}
#[inline]
...
pub fn new() -> CurveEditor {
        ...
        CurveEditor { width: 300_i32, height: 200_i32, points, min_value: 0.0_f32, max_value: 1.0_f32, grid_enabled: true, on_change: "".to_string() }
}
#[inline]
...
pub fn apply_preset(&mut self, preset: EasingPreset) {
        ...
        match preset {
            ...
            EasingPreset::EaseIn => {
                self.points.push(CurvePoint::new(0.0_f32, 0.0_f32).tangents(0.0_f32, 0.5_f32));
                self.points.push(CurvePoint::new(1.0_f32, 1.0_f32).tangents(0.5_f32, 0.0_f32));
            },
            EasingPreset::EaseOut => {
                self.points.push(CurvePoint::new(0.0_f32, 0.0_f32).tangents(0.0_f32, 2.0_f32));
                self.points.push(CurvePoint::new(1.0_f32, 1.0_f32).tangents(2.0_f32, 0.0_f32));
            },
...
fn render(&self) -> String {
        ...
        for i in 0_usize..self.points.len() {
            ...
            match p {
                Some(point) => {
                    ...
                    let y = self.height as f32 - (((point.value - self.min_value) / range) as f32 * self.height as f32) as f32;
                    if i == 0 {
                        path_d = format!("M {} {}", x, y);
                    } else {
        ...
        let mut points_html = String::new();
        for i in 0_usize..self.points.len() {
            let p = self.points.get(i);
            match p {
                Some(point) => {
...
pub fn new(position: f32, color: String) -> GradientStop {
        GradientStop { position, color: color.to_string() }
}
}
...
pub fn new() -> GradientEditor {
        ...
        GradientEditor { width: 300_i32, height: 40_i32, stops, on_change: "".to_string() }
}
#[inline]
        ...
        for i in 0_usize..self.stops.len() {
            ...
            match s {
                Some(stop) => {
                    ...
                    gradient_stops = gradient_stops + &stop.color.clone() + &" ".to_string() + &format!("{}%", (stop.position * 100.0_f32) as i32);
                },
                None => {
                },
            }
        }
        let mut markers_html = String::new();
        for i in 0_usize..self.stops.len() {
            let s = self.stops.get(i);
            match s {
                Some(stop) => {
